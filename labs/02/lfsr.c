...
unsigned getbit(uint16_t *x, unsigned n) {
  return (*x & (1 << n)) >> n;
}

void setbit(uint16_t *x, unsigned n, unsigned v) {
  uint16_t mask = ~(1 << n) | (v << n);
  *x &= mask;
  *x |= (v << n);
}
...
void lfsr_calculate(uint16_t *reg) {
  /* YOUR CODE HERE */
  unsigned shift_bit = getbit(reg, 5) ^ (getbit(reg, 3) ^ (getbit(reg, 2) ^ getbit(reg, 0)));
  *reg >>= 1;
  setbit(reg, 15, shift_bit);
}
